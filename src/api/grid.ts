...
export const post_smartParams = (body: { symbol: string; investment_usdt?: string }) =>
  AzAxios.post(`${BASE}/smart-params`, body) as Promise<SmartParamsRes>;

export const post_profitPreview = (body: {
  symbol: string;
  lower_price: string;
  upper_price: string;
  grid_count: number;
...
export const get_balance = (asset = "USDT") =>
  AzAxios.get(`${BASE}/account/balance`, { params: { asset }, ...withUid() }) as Promise<BalanceRes>;

export const post_deposit = (body: { asset: string; amount: string }) =>
  AzAxios.post(`${BASE}/balance/deposit`, body, withUid());

export const post_withdrawToMain = (body: { asset: string; amount: string }) =>
  AzAxios.post(`${BASE}/balance/withdraw-to-main`, body, withUid());

export const post_createBot = (body: CreateBotReq) =>
  AzAxios.post(`${BASE}/bots`, body, withUid()) as Promise<CreateBotRes>;
...
export const get_botOverview = (botId: string) =>
  AzAxios.get(`${BASE}/bots/${botId}/overview`, withUid());

export const get_botHoldingDetail = (botId: string) =>
  AzAxios.get(`${BASE}/bots/${botId}/holding-detail`, withUid());

export const get_botGridOrders = (botId: string) =>
  AzAxios.get(`${BASE}/bots/${botId}/grid-orders`, withUid());

export const get_botParams = (botId: string) =>
  AzAxios.get(`${BASE}/bots/${botId}/params`, withUid());

export const get_botTrades = (botId: string) =>
  AzAxios.get(`${BASE}/bots/${botId}/trades`, withUid());

export const patch_botName = (botId: string, body: { name: string }) =>
  AzAxios.patch(`${BASE}/bots/${botId}/name`, body, withUid());

export const post_botPause = (botId: string) =>
  AzAxios.post(`${BASE}/bots/${botId}/pause`, undefined, withUid());
...
export const post_botProfitWithdraw = (botId: string, body: { amount_usdt?: string }) =>
  AzAxios.post(`${BASE}/bots/${botId}/profit/withdraw`, body, withUid());
...
export const post_botAddInvestment = (botId: string, body: { amount_usdt: string }) =>
  AzAxios.post(`${BASE}/bots/${botId}/add-investment`, body, withUid());
